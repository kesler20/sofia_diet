...
async function mockOCRProcess(_file: File): Promise<Receipt> {
...
export default function ReceiptSplitManager() {
  ...
  const [people, setPeople] = React.useState<Person[]>([
    { id: "1", name: "Alice", email: "alice@email.com", color: "#5b9bd5" },
    { id: "2", name: "Bob", email: "bob@email.com", color: "#48a999" },
  ]);
  ...
  const handleEventUploadReceipt = async (
    ...
    setIsUploading(true);
    try {
      const newReceipt = await mockOCRProcess(file);
