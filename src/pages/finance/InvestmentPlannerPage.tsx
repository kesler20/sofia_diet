...
const SAMPLE_BUDGET_DATA: Record<Months, number> = {
  January: 1200, February: 1100, March: 1300, April: 1150,
  May: 1250, June: 1400, July: 1350, August: 1200,
  September: 1100, October: 1250, November: 1300, December: 800,
};
    ...
    <Card className="p-4 py-6">
      ...
      <ResponsiveContainer width="100%" height={380}>
        <BarChart data={chartData} margin={{ top: 20, right: 20, left: 0, bottom: 5 }}>
          ...
          <Bar dataKey="OpEx" stackId="a" fill="#f97316" radius={[0, 0, 0, 0]} />
          <Bar dataKey="Investment" stackId="a" fill="#22c55e" radius={[0, 0, 0, 0]} />
...
function MonthlyTable(props: { data: MonthlyAllocation[] }) {
  const totals = props.data.reduce((acc, d) => ({
  ...
  }), { available: 0, opex: 0, investment: 0, gifts: 0 });
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b-2 border-gray-200">
            <th className="text-left py-3 px-3 text-xs font-bold text-gray-500 uppercase tracking-wide">Month</th>
            <th className="text-right py-3 px-3 text-xs font-bold text-gray-500 uppercase tracking-wide">Available</th>
            <th className="text-right py-3 px-3 text-xs font-bold text-orange-500 uppercase tracking-wide">OpEx</th>
            <th className="text-right py-3 px-3 text-xs font-bold text-green-600 uppercase tracking-wide">Invest</th>
            <th className="text-right py-3 px-3 text-xs font-bold text-purple-500 uppercase tracking-wide">Gifts</th>
          </tr>
        </thead>
        <tbody>
          {props.data.map((row, i) => (
            <tr key={row.month} className={i % 2 === 0 ? "bg-gray-50/50" : "bg-white"}>
              <td className="py-2.5 px-3 font-semibold text-gray-700">{row.month}</td>
              <td className="py-2.5 px-3 text-right font-medium text-gray-500">£{row.availableFunds.toFixed(0)}</td>
              <td className="py-2.5 px-3 text-right font-semibold text-orange-600">£{row.opex.toFixed(0)}</td>
              <td className="py-2.5 px-3 text-right font-semibold text-green-600">£{row.investment.toFixed(0)}</td>
              <td className="py-2.5 px-3 text-right font-semibold text-purple-600">£{row.gifts.toFixed(0)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr className="border-t-2 border-gray-300 bg-gray-100">
            ...
            <td className="py-3 px-3 text-right font-bold text-gray-600">£{totals.available.toFixed(0)}</td>
            <td className="py-3 px-3 text-right font-bold text-orange-600">£{totals.opex.toFixed(0)}</td>
            <td className="py-3 px-3 text-right font-bold text-green-600">£{totals.investment.toFixed(0)}</td>
            <td className="py-3 px-3 text-right font-bold text-purple-600">£{totals.gifts.toFixed(0)}</td>
          </tr>
        </tfoot>
      </table>
    </Card>
  ...
  const projectionData = React.useMemo(() => {
    ...
    for (let year = 0; year <= props.params.years; year++) {
      ...
      const calcFV = (annualRate: number) => {
        ...
        const realRate = (1 + monthlyRate) / (1 + props.params.inflationRate / 12) - 1;
        const totalRate = realRate + props.params.dividendYield / 12;
        if (months === 0) return props.params.principal;
        return props.params.principal * Math.pow(1 + totalRate, months) +
          props.params.monthlyContribution * ((Math.pow(1 + totalRate, months) - 1) / totalRate);
      };
      data.push({ year, pessimistic: calcFV(0.07), expected: calcFV(0.10), optimistic: calcFV(0.12) });
    }
    return data;
  }, [props.params]);
    ...
    <Card className="p-4 py-6">
        ...
        <AreaChart data={projectionData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
          ...
          <Tooltip
            ...
            formatter={(v: number, name: string) => [
              ...
              name === "optimistic" ? "Best (12%)" : name === "expected" ? "Expected (10%)" : "Conservative (7%)"
            ]}
            labelFormatter={(l) => `Year ${l}`}
          />
          <Area type="monotone" dataKey="optimistic" stroke="#22c55e" strokeWidth={2} fill="url(#optG)" />
          <Area type="monotone" dataKey="expected" stroke="#3b82f6" strokeWidth={2.5} fill="url(#expG)" />
          <Area type="monotone" dataKey="pessimistic" stroke="#f97316" strokeWidth={2} fill="url(#pesG)" />
        </AreaChart>
      </ResponsiveContainer>
      ...
      <div className="grid grid-cols-3 gap-3 mt-5">
        <div className="bg-orange-50 rounded-xl p-3 border border-orange-200">
          <p className="text-[10px] font-bold text-orange-600 uppercase tracking-wide">Conservative 7%</p>
          <p className="text-lg font-bold text-orange-700">
            £{final?.pessimistic.toLocaleString("en-GB", { maximumFractionDigits: 0 })}
          </p>
        </div>
        <div className="bg-blue-50 rounded-xl p-3 border border-blue-200">
          <p className="text-[10px] font-bold text-blue-600 uppercase tracking-wide">Expected 10%</p>
          <p className="text-lg font-bold text-blue-700">
            £{final?.expected.toLocaleString("en-GB", { maximumFractionDigits: 0 })}
          </p>
        </div>
        <div className="bg-green-50 rounded-xl p-3 border border-green-200">
          <p className="text-[10px] font-bold text-green-600 uppercase tracking-wide">Best Case 12%</p>
      ...
      <div className="mt-4 p-3 bg-gray-50 rounded-xl border border-gray-200 text-xs text-gray-500">
        <span className="font-semibold">Note:</span> Adjusted for UK inflation ({(props.params.inflationRate * 100).toFixed(1)}%) with reinvested dividends ({(DIVIDEND_YIELD * 100).toFixed(1)}% yield). Shows real purchasing power.
      </div>
    </Card>
  );
}
      ...
      <div className="flex flex-col md:flex-row md:items-end gap-4">
        ...
        <div className="flex gap-3 flex-wrap flex-1 md:justify-end">
          <div className="bg-gray-50 rounded-xl px-4 py-2.5 border border-gray-200">
            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wide">Avg Available</p>
            <p className="text-lg font-bold text-gray-700">£{(props.totalAvailable / 12).toFixed(0)}</p>
          </div>
          <div className="bg-orange-50 rounded-xl px-4 py-2.5 border border-orange-200">
            <p className="text-[10px] font-bold text-orange-500 uppercase tracking-wide">Avg OpEx</p>
            <p className="text-lg font-bold text-orange-600">£{(props.totalOpex / 12).toFixed(0)}</p>
          </div>
          <div className="bg-purple-50 rounded-xl px-4 py-2.5 border border-purple-200">
            <p className="text-[10px] font-bold text-purple-500 uppercase tracking-wide">Avg Gifts</p>
            <p className="text-lg font-bold text-purple-600">£{(props.totalGifts / 12).toFixed(0)}</p>
          </div>
        </div>
      </div>
    </Card>
...
export default function InvestmentPlannerPage() {
  ...
  const [fixedInvestment, setFixedInvestment] = React.useState<number>(500);
  const [monthlyBudgetData] = React.useState<Record<Months, number>>(SAMPLE_BUDGET_DATA);
  const [investmentParams, setInvestmentParams] = React.useState<InvestmentParams>({
    principal: 10000,
  ...
  const calculateAllocations = (): MonthlyAllocation[] => {
    return MONTHS.map((month) => {
      ...
      const remaining = available - investment;
      const opex = remaining * 0.8;
      const gifts = remaining * 0.2;
      return { month, availableFunds: available, opex, investment, gifts };
    });
  };
  ...
  const handleEventChangeInvestment = (value: number) => {
    setFixedInvestment(value);
    setInvestmentParams((prev) => ({ ...prev, monthlyContribution: value }));
  };

  const handleEventChangeInvestmentParam = (field: keyof InvestmentParams, value: number) => {
    setInvestmentParams((prev) => ({ ...prev, [field]: value }));
    if (field === "monthlyContribution") setFixedInvestment(value);
  };
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex justify-center mb-6">
          <ViewToggle activeView={activeView} onEventChangeView={handleEventChangeView} />
        </div>
        ...
        {activeView === "forecast" && (
          <ForecastChart params={investmentParams} onEventChangeParam={handleEventChangeInvestmentParam} />
        )}
      </div>
    </div>
  );
