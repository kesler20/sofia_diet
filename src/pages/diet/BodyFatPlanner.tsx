...
export default function BodyFatPlanner() {
  ...
  const [goalDraft, setGoalDraft] = React.useState<GoalDraft>(() => {
    const restored = readFromLocalStorage<GoalDraft>("bf.goalDraft");
    return (
      restored ?? {
        unit: "kg",
        currentWeight: 85,
  ...
  const [activeGoal, setActiveGoal] = React.useState<ActiveGoal | null>(() =>
    readFromLocalStorage<ActiveGoal>("bf.activeGoal")
  );

  const [checkIns, setCheckIns] = React.useState<CheckIn[]>(
    () => readFromLocalStorage<CheckIn[]>("bf.checkIns") ?? []
  );
  ...
  React.useEffect(() => {
    writeToLocalStorage("bf.goalDraft", goalDraft);
  }, [goalDraft]);

  React.useEffect(() => {
    writeToLocalStorage("bf.activeGoal", activeGoal);
  }, [activeGoal]);

  React.useEffect(() => {
    writeToLocalStorage("bf.checkIns", checkIns);
  }, [checkIns]);
  ...
  const handleEventClearAllData = () => {
    setActiveGoal(null);
    setCheckIns([]);
    deleteFromLocalStorage("bf.activeGoal");
    deleteFromLocalStorage("bf.checkIns");
  };
